...
legend = 'status        i1:count() i2:count()'
legend = legend .. ' tx1:i1:count() tx1:i2:count()'
legend = legend .. ' tx2:i1:count() tx2:i2:count()'
function check()                                                         \
    local res = ''                                                       \
    local ok = true                                                      \
    ...
    if (tx1('i1:count()')[1] ~= tx1('#i1:select{}')[1]) or               \
       (tx1('i1:len()')[1] ~= tx1('#i1:select{}')[1]) then               \
        ok = false                                                       \
        res = res .. '!'                                                 \
    end                                                                  \
    res = res .. '        ' .. tostring(tx1('i2:count()')[1])            \
    if (tx1('i2:count()')[1] ~= tx1('#i2:select{}')[1]) or               \
       (tx1('i2:len()')[1] ~= tx1('#i2:select{}')[1]) then               \
        ok = false                                                       \
        res = res .. '!'                                                 \
    end                                                                  \
    res = res .. '        ' .. tostring(tx2('i1:count()')[1])            \
    if (tx2('i1:count()')[1] ~= tx2('#i1:select{}')[1]) or               \
       (tx2('i1:len()')[1] ~= tx2('#i1:select{}')[1]) then               \
        ok = false                                                       \
        res = res .. '!'                                                 \
    end                                                                  \
    res = res .. '        ' .. tostring(tx2('i2:count()')[1])            \
    if (tx2('i2:count()')[1] ~= tx2('#i2:select{}')[1]) or               \
       (tx2('i2:len()')[1] ~= tx2('#i2:select{}')[1]) then               \
        ok = false                                                       \
        res = res .. '!'                                                 \
    end                                                                  \
									 \
...
function background_mvcc()                                              \
    while run_background_mvcc do                                        \
        local bal = math.ceil(math.random() * 200) - 100                \
        box.space.accounts:update('petya', {{'+', 'balance', bal}})     \
    end                                                                 \
end
